...
const DataTable: React.FC<DataTableProps> = ({
  ...
  const handleDownloadCsvInternal = () => {
    ...
    const csvFilename = (filename ? filename.substring(0, filename.lastIndexOf('.')) || filename : 'data') + '.csv';
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
        ...
        <div>
          <input
            ...
            id="csvUploadInput"
          />
          <label htmlFor="csvUploadInput" className="csv-button" style={{ marginRight: '10px', cursor: 'pointer', padding: '5px 10px', border: '1px solid #ccc', borderRadius: '4px', backgroundColor: '#f0f0f0' }}>
            Upload CSV
          </label>
...
const parseCSV = (csvString: string, currentTouchstone: Touchstone): { matrix: Complex[][][], frequencies: number[] } => {
  ...
  const dataLines = lines.slice(1).map(line => line.trim()).filter(line => line);
  ...
  if (actualHeaders.length !== expectedHeaderParts.length) {
    throw new Error(`CSV header count mismatch. Expected ${expectedHeaderParts.length} columns, got ${actualHeaders.length}.`);
  }
  for (let i = 0; i < actualHeaders.length; i++) {
    if (actualHeaders[i].trim() !== expectedHeaderParts[i].trim()) {
      throw new Error(`CSV header mismatch at column ${i + 1}. Expected "${expectedHeaderParts[i]}", got "${actualHeaders[i]}".`);
    }
  }
  ...
  const matrixData: Complex[][][] = Array(nports).fill(null).map(() => Array(nports).fill(null).map(() => []));

  for (const line of dataLines) {
    const values = line.split(',').map(v => parseFloat(v.trim()));
    if (values.length !== expectedHeaderParts.length) {
      throw new Error(`CSV data row has incorrect number of values. Expected ${expectedHeaderParts.length}, got ${values.length}. Line: "${line}"`);
    }
    if (values.slice(1).some(isNaN)) { // Check S-param values for NaN
        throw new Error(`CSV data row contains non-numeric S-parameter values. Line: "${line}"`);
    }
    if (isNaN(values[0])) { // Check frequency value for NaN
        throw new Error(`CSV data row contains non-numeric frequency value. Line: "${line}"`);
    }
      ...
      for (let inPortIdx = 0; inPortIdx < nports; inPortIdx++) {
        ...
        switch (currentFormat) {
          ...
          case 'DB':
            ...
            break;
          default:
            throw new Error(`Unsupported format for CSV parsing: ${currentFormat}`);
        }
 ...
 */
const formatDataValues = (param: Complex | undefined, format: string | undefined, precision: number = 4): { value1: string; value2: string } => {
