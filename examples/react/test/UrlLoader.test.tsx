...
describe('UrlLoader Component', () => {
  ...
  test('renders "Open from URL" button initially', () => {
    ...
    expect(screen.getByRole('button', { name: /Open from URL/i })).toBeInTheDocument();
  });
  ...
  test('shows URL input and "Load URL" button when "Open from URL" is clicked', () => {
    ...
    expect(screen.getByPlaceholderText(/Enter Touchstone file URL/i)).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Load URL/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Cancel URL Load/i })).toBeInTheDocument(); // Button text changes
  });
  ...
  test('hides URL input and "Load URL" button when "Cancel URL Load" is clicked', () => {
    ...
    const cancelButton = screen.getByRole('button', { name: /Cancel URL Load/i });
    fireEvent.click(cancelButton); // Hide inputs

    expect(screen.queryByPlaceholderText(/Enter Touchstone file URL/i)).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /Load URL/i })).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Open from URL/i })).toBeInTheDocument(); // Button text reverts
  });
  ...
  test('updates URL input value on change', () => {
    ...
    const urlInput = screen.getByPlaceholderText(/Enter Touchstone file URL/i) as HTMLInputElement;
    fireEvent.change(urlInput, { target: { value: 'http://example.com/test.s2p' } });
  ...
  test('calls onUrlSubmit with the URL when "Load URL" is clicked with a valid URL', () => {
    ...
    fireEvent.change(urlInput, { target: { value: 'https://example.com/data.s2p' } });
  ...
  test('clears error message when input changes after an error was shown', () => {
   ...
   test('clears URL and error when "Cancel URL Load" is clicked after an error', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Open from URL/i }));
    const freshUrlInput = screen.getByPlaceholderText(/Enter Touchstone file URL/i) as HTMLInputElement;
