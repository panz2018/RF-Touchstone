        ...
        <button
          onClick={() => {
            if (mockTriggerUrlSubmit)
              mockTriggerUrlSubmit('http://default-mock-url.com/file.s2p')
          }}
        >
...
// Mock DataTable
let mockTriggerMatrixUpdateFromCsv:
  | ((matrix: Complex[][][], frequencies: number[], filename: string) => void)
  | null = null
vi.mock('../src/components/DataTable', () => ({
  default: vi.fn((props) => {
    ...
    mockTriggerMatrixUpdateFromCsv = (
      matrix,
      frequencies,
      _newFilename_not_used
    ) => {
  ...
  default: vi.fn((props) => {
    ...
    return (
      <div data-testid="mock-fileinfo">
        ...
        <button
          data-testid="mock-fileinfo-set-unit"
          onClick={() => props.setUnit('MockNewUnit')}
        />
        <button
          data-testid="mock-fileinfo-set-format"
          onClick={() => props.setFormat('MockNewFormat')}
        />
        <button
          data-testid="mock-fileinfo-set-comments"
          onClick={() => props.setComments(['MockNewComment'])}
        />
        <button
          data-testid="mock-fileinfo-set-impedance"
          onClick={() => props.setImpedance(75)}
        />
      </div>
    )
  }),
...
vi.mock('../src/components/DownloadButton', () => ({
  default: vi.fn(({ touchstone, filename }) => (
    <button data-testid="mock-downloadbutton" disabled={!touchstone}>
      Mock Download Button for {filename} (Data:{' '}
      {touchstone ? 'Present' : 'Absent'})
    </button>
  )),
}))
...
describe('TouchstoneViewer Component', () => {
  ...
  beforeEach(() => {
    ...
    mockedReadUrl.mockImplementation(async (url: string) => {
      ...
      return Promise.reject(
        new Error(`mockReadUrl received unexpected URL: ${url}.`)
      )
    })
  ...
  it('handles filename change from FileInfo', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(`Mock Download Button for ${newFilenameFromChild}`)
      ).toBeInTheDocument()
      expect(
  ...
  it('handles comments change from FileInfo and updates touchstone object for copy/download', async () => {
    ...
    expect(copyButtonProps.touchstone.comments).toEqual(
      initialLoadedTs.comments
    )
    ...
    if (mockFileInfoHandlers.setComments) {
      act(() => {
        mockFileInfoHandlers.setComments(newCommentsFromChild)
      })
    }
    ...
    await waitFor(() => {
      ...
      const downloadButtonProps = MockedDownloadButton.mock.lastCall![0] as any
      expect(downloadButtonProps.touchstone.comments).toEqual(
        newCommentsFromChild
      )
    })
    ...
    await waitFor(() => {
      expect(
        screen.getByText(`Comments: ${newCommentsFromChild.join('/')}`)
      ).toBeInTheDocument()
  ...
  it('handles URL submission, loads data, and updates children props', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Currently displaying: Data from data.s1p/i)
      ).toBeInTheDocument()
  ...
  it('sets an error if filename cannot be derived from URL in loadFileContent', async () => {
    ...
    if (mockTriggerUrlSubmit) {
      act(() => {
        mockTriggerUrlSubmit(invalidUrl)
      })
    }
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          `Error: Could not determine a valid filename from URL: ${invalidUrl}`
        )
      ).toBeInTheDocument()
    })
    expect(mockedReadUrl).not.toHaveBeenCalled()
  ...
  it('updates touchstone matrix and filename when CSV data is uploaded via DataTable', async () => {
    ...
    if (mockTriggerMatrixUpdateFromCsv) {
      act(() => {
        ...
        mockTriggerMatrixUpdateFromCsv(
          newMatrix,
          newFrequencies,
          filenameUsedByDataTableForDisplay
        )
      })
    }
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(`Currently displaying: Data from ${originalFilename}`)
      ).toBeInTheDocument()
      ...
      expect(dataTableProps.touchstone.frequency.f_Hz).toEqual(
        newFrequencies.map(
          (f) =>
            f *
            dataTableProps.touchstone.frequency.getMultiplier(
              dataTableProps.touchstone.frequency.unit
            )
        )
      )
  ...
  it('updateMatrixFrequency sets error if base touchstone data is not loaded', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Error: Initial load failed for test/i)
      ).toBeInTheDocument()
    })
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          'Error: Cannot update matrix/frequency: No base Touchstone data is currently loaded.'
        )
      ).toBeInTheDocument()
    })
  ...
  describe('Setter Error Handling when touchstone is null', () => {
    ...
    it('setUnit throws error if touchstone is null', async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByText(/Error: Simulated initial load failure/i)
        ).toBeInTheDocument()
      )
      ...
      const fileInfoProps = MockedFileInfo.mock.calls[0][0] as any
      expect(() => fileInfoProps.setUnit('GHz')).toThrow(
    ...
    it('setFormat throws error if touchstone is null', async () => {
      ...
      const fileInfoProps = MockedFileInfo.mock.calls[0][0] as any
      expect(() => fileInfoProps.setFormat('MA')).toThrow(
        'Cannot set format: No Touchstone data loaded.'
      )
    })
    ...
    it('setComments throws error if touchstone is null', async () => {
      ...
      const fileInfoProps = MockedFileInfo.mock.calls[0][0] as any
      expect(() => fileInfoProps.setComments(['new comment'])).toThrow(
        'Cannot set comments: No Touchstone data loaded.'
      )
    })
    ...
    it('setImpedance throws error if touchstone is null', async () => {
      ...
      const fileInfoProps = MockedFileInfo.mock.calls[0][0] as any
      expect(() => fileInfoProps.setImpedance(75)).toThrow(
        'Cannot set impedance: No Touchstone data loaded.'
      )
    })
