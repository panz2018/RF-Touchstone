# React Example: Touchstone Converter

**[🚀 View Live Demo](https://panz2018.github.io/RF-Touchstone/examples/react/index.html)**

[![React Example Tests](https://github.com/panz2018/RF-Touchstone/actions/workflows/test-react-example.yml/badge.svg?event=pull_request)](https://github.com/panz2018/RF-Touchstone/actions/workflows/test-react-example.yml)

This example demonstrates how to build a complete **Touchstone Converter** using the `rf-touchstone` library. Built with React 19, Vite, and TypeScript. It allows users to upload a Touchstone file (e.g., `.s1p`, `.s2p`) and view its parsed contents.
...
This example imports `rf-touchstone` directly and utilizes its static helper methods (like `Touchstone.fromUrl`, `Touchstone.fromFile`, and `Touchstone.fromText`) for simplified parsing:

```typescript
import { Touchstone } from 'rf-touchstone';

// Parse from a URL
const touchstone = await Touchstone.fromUrl('sample.s2p');

// Parse from a File object (e.g., from a file input)
const touchstoneFromFile = await Touchstone.fromFile(file);

// Parse from raw text content
const touchstoneFromText = Touchstone.fromText(textContent, 2);
```
