...
} from '../src/frequency'; // Adjusted path based on typical project structure

// Helper data for looped tests
const testBaseUnits: FrequencyUnits[] = ['Hz', 'kHz', 'MHz', 'GHz'];
...
describe('frequency.ts', () => {
  ...
  it('Frequency:unit', () => {
    ...
    // Wrong input type
    expect(() => (frequency.unit = 0 as never)).toThrowError(/Unknown frequency unit: 0/);
    expect(() => (frequency.unit = undefined as never)).toThrowError(/Unknown frequency unit: undefined/);
    expect(() => (frequency.unit = null as never)).toThrowError(/Unknown frequency unit: null/);
    
    // Wrong input value
    expect(() => (frequency.unit = 'z' as never)).toThrowError(/Unknown frequency unit: z/);
    expect(() => (frequency.unit = '' as never)).toThrowError(/Unknown frequency unit: /); // Empty string
  });
  ...
  it('Frequency:f_scaled', () => {
    ...
    // Wrong input type
    expect(() => (frequency.f_scaled = 0 as never)).toThrowError('Frequency value must be an array');
    expect(() => (frequency.f_scaled = 'test' as never)).toThrowError('Frequency value must be an array');
    
    // Wrong array element type
    expect(() => (frequency.f_scaled = ['1', '2', '3'] as never[])).toThrowError('Frequency value must be an array of numbers');
    
    // Negative values
    expect(() => (frequency.f_scaled = [1, 2, -3])).toThrowError('Frequency values cannot be negative');
  });
  ...
  freqAccessors.forEach(gs => {
    testBaseUnits.forEach(unit => {
        it(`${gs.name} Setter/Getter: Set as ${gs.name}, Get as ${gs.name}, Current unit ${unit}`, () => {
            ...
            retrievedValues.forEach((val, i) => expect(val).toBeCloseTo(valuesToSet[i], 5));
        });
    });
    ...
    it(`${gs.name} Setter: Overwrite with different length array`, () => {
        ...
        retrievedValues.forEach((val, i) => expect(val).toBeCloseTo(newValuesToSet[i], 5));

        // 2. Check if f_scaled (which is in Hz) reflects these new values, converted to Hz
        const expectedFScaledInHz = newValuesToSet.map(v => (v * FREQUENCY_MULTIPLIERS[gs.key]) / FREQUENCY_MULTIPLIERS['Hz']);
        expect(freq.f_scaled.length).toBe(expectedFScaledInHz.length);
        freq.f_scaled.forEach((val, i) => expect(val).toBeCloseTo(expectedFScaledInHz[i], 5));
    });
  ...
  it("Freq Get/Set: Cross-unit f_kHz setter to GHz base", () => {
      ...
      freq.f_scaled.forEach((val, i) => expect(val).toBeCloseTo(expectedGHzInFScaled[i], 5));
  });
  ...
  it("Freq Get/Set: Cross-unit f_Hz setter to kHz base", () => {
      ...
      freq.f_scaled.forEach((val, i) => expect(val).toBeCloseTo(expectedKHzInFScaled[i], 5));
  });
  ...
  wavelengthAccessors.forEach(wgs => {
      ...
      it(`${wgs.name} Setter: Overwrite with different length array`, () => {
          ...
          retrievedWavelengths.forEach((val, i) => expect(val).toBeCloseTo(newWavelengthsToSet[i], 5));
          
          // 2. Check if f_scaled (which is in Hz) reflects these new wavelengths, converted to Hz
          const expectedFScaledInHz = newWavelengthsToSet.map(wl => 
              SPEED_OF_LIGHT / (wl * WAVELENGTH_MULTIPLIERS_TO_M[wgs.key])
          );
          expect(freq.f_scaled.length).toBe(expectedFScaledInHz.length);
          freq.f_scaled.forEach((val, i) => expect(val).toBeCloseTo(expectedFScaledInHz[i], 0)); // Precision 0 for Hz if large numbers
      });
  ...
  it("Wavelength Get/Set: Cross-type wavelength_m getter from MHz base", () => {
      ...
      wavelengths.forEach((val, i) => expect(val).toBeCloseTo(expectedWavelengths_m[i], 5));
  });
  ...
  it("Wavelength Get/Set: Cross-type wavelength_cm setter to MHz base", () => {
      ...
      const expectedFreqs_MHz = [
          (SPEED_OF_LIGHT / (10 * WAVELENGTH_MULTIPLIERS_TO_M['cm'])) / FREQUENCY_MULTIPLIERS['MHz'],
          (SPEED_OF_LIGHT / (20 * WAVELENGTH_MULTIPLIERS_TO_M['cm'])) / FREQUENCY_MULTIPLIERS['MHz']
      ];
      expect(freq.f_scaled.length).toBe(expectedFreqs_MHz.length);
      freq.f_scaled.forEach((val, i) => expect(val).toBeCloseTo(expectedFreqs_MHz[i], 3));
  });
